---
title: "How to Plan a London Itinerary That Actually Works"
seoTitle: "London Itinerary Planner: How Many Days You Need & What to Book"
description: "How many days London really needs, which plan to follow, what to book before you fly, and the planning rules that stop an itinerary falling apart."
publishedAt: 2026-07-28
updatedAt: 2026-08-27
sites: [london]
canonicalSite: london
category: "London itineraries"
tags: [itineraries, trip planning, first visit]
draft: false
faq:
  - q: "How many days do you need in London?"
    a: "Three days covers the essentials for a first visit — Westminster, the Tower and South Bank, and one museum district. Five is where it stops feeling rushed and you get Greenwich and a full neighbourhood day. Beyond a week you are choosing between London and day trips, which is a different decision."
  - q: "What is the biggest mistake in a London itinerary?"
    a: "Grouping by fame instead of by geography. The famous sights are scattered, so a list-driven plan sends you across the city repeatedly. Grouping by neighbourhood removes almost all of the transport and adds hours back to the trip."
  - q: "What should I book before travelling to London?"
    a: "The Tower of London and Westminster Abbey both use timed entry and sell out at peak times, any popular West End show needs two to four weeks, and Horizon 22 releases free slots weeks ahead. Almost everything else, including the free museums, can be decided on the day."
  - q: "Do I need a London Travelcard or Oyster card?"
    a: "Usually neither. A contactless bank card taps in the same way and hits the same daily cap, so most visitors need nothing extra. See our fares guide for the exceptions."
  - q: "How much walking should I plan for in London?"
    a: "A well-grouped London day is four to six miles, almost all flat. That is more than most people expect and it is the reason ambitious itineraries collapse by mid-afternoon. Build in one empty hour a day."
heroImage: "../../assets/articles/london-itineraries-by-days-and-interests/london-itineraries-by-days-and-interests.jpg"
heroImageAlt: "A London street map view with red buses crossing Westminster Bridge"
---

The best London itinerary is not the one with the most on it. It is the one that groups nearby places together and leaves room for queues, meals and the thing you did not plan.

The city's famous sights are scattered across a wide area, so a plan built from a list of landmarks will send you back and forth across the centre. **Grouping by geography instead of by fame is worth about two hours a day.**

## Choose your plan

| Time available | Guide | What it covers |
| --- | --- | --- |
| **One day** | [Six one-day plans by interest](/articles/one-day-london-itineraries-by-interest/) | Pick a theme — first-timer, theatre, museums, families, food, or low-walking |
| **Three days** | [Three days in London](/articles/three-days-in-london-itinerary/) | Westminster, the Tower and South Bank, one museum district |
| **Five days** | [Five days in London](/articles/five-days-in-london-itinerary/) | The above, plus Greenwich by river and a full neighbourhood day |

---

## How many days does London actually need?

* **One day** — enough for one theme done properly. Not enough for "the highlights", and trying will cost you a third of the day underground.
* **Two days** — Westminster and the West End on one, the Tower and South Bank on the other. Skip the museums entirely rather than rushing them.
* **Three days** — the first length that covers a first visit honestly. Landmarks, river and one museum district.
* **Five days** — where it stops being a race. Adds Greenwich and a whole day in one neighbourhood, which is usually the day people remember.
* **A week or more** — now you are choosing between more London and a day trip to Windsor, Oxford or Bath. Both are defensible; doing both badly is not.

---

## Book before you fly

Four things genuinely sell out. Everything else can wait.

| Book | How far ahead | Why |
| --- | --- | --- |
| **Tower of London** | 2–3 weeks in peak season | Timed entry, and the Crown Jewels queue grows all day. Take the first slot |
| **Westminster Abbey** | 1–2 weeks | Timed entry, and **closed to sightseers on Sundays** |
| **A West End show** | 2–4 weeks | Direct from the theatre, not a reseller |
| **Horizon 22** | Weeks ahead | Free, and the highest free viewpoint in London |

**Almost everything else is free and needs no booking** — the British Museum, National Gallery, Tate Modern, V&A, Natural History Museum, Science Museum and British Library all charge nothing to walk in.

---

<div data-gyg-href="https://widget.getyourguide.com/default/activities.frame" data-gyg-locale-code="en-US" data-gyg-widget="activities" data-gyg-number-of-items="3" data-gyg-cmp="london-itineraries-book-ahead" data-gyg-partner-id="WWP7I0R" data-gyg-q="Westminster Abbey tour"><span>Powered by <a target="_blank" rel="sponsored" href="https://www.getyourguide.com/london-l57/">GetYourGuide</a></span></div>

## Planning rules

These are the ones that hold a plan together when it meets reality.

- **Group by neighbourhood, not by landmark.** The single highest-value rule here.
- **One timed attraction per half-day at most.** Two anchors in an afternoon means watching a clock instead of a city.
- **Treat meals as part of the itinerary**, not an interruption. Ten minutes' walk from any major sight halves the price and improves the food.
- **Check closing days.** Borough Market shuts Sunday, Westminster Abbey closes to sightseers Sunday, and several museums close one day a week.
- **Build a wet-weather alternative** for each day. See [London in the rain](/articles/london-in-the-rain/).
- **Leave one hour a day genuinely empty.** It is what absorbs a delayed train or a longer queue.
- **Allow more time than the journey planner says** if you are travelling with children, luggage or a step-free route.
- **Do not book dinner five minutes after the previous thing ends** — and never book a curtain-up an hour after a restaurant reservation.

---

## Getting around while you do it

Most visitors need no ticket at all beyond the bank card in their pocket. Contactless taps in and out the same as an Oyster and hits the same daily cap.

- **[Transport costs and fares](/articles/london-public-transport-costs-and-fares/)** — what a day actually costs, and how the cap works
- **[Oyster card guide](/articles/oyster-card-guide-london/)** — and when you genuinely need one
- **[Getting around London](/articles/getting-around-london-transport-guide/)** — the overview
- **[River boats](/articles/how-to-use-london-river-boats/)** — the most underrated way to move, and the best approach to Greenwich

---

## Where to base yourself

Where you sleep changes the itinerary more than anything else on this page. A central base removes an hour of travel a day.

Use the [best areas guide](/articles/best-areas-to-visit-london/) to choose, but broadly: [Covent Garden](/articles/covent-garden-area-guide/), [Soho](/articles/soho-area-guide/) and the [South Bank](/articles/south-bank-area-guide/) for walking; [King's Cross](/articles/kings-cross-area-guide/) and [Bloomsbury](/articles/bloomsbury-area-guide/) for rail links and better value.

---

## Continue planning your London trip

- 🗓️ **[Three Days in London](/articles/three-days-in-london-itinerary/)**
- 🗓️ **[Five Days in London](/articles/five-days-in-london-itinerary/)**
- 🎯 **[One-Day Plans by Interest](/articles/one-day-london-itineraries-by-interest/)**
- 🏘️ **[Best London Areas to Visit](/articles/best-areas-to-visit-london/)**
- 💷 **[London on a Budget](/articles/london-on-a-budget/)**
- 🎪 **[Free Things to Do in London](/free/)**

---

*Check opening days, market days and transport status before travelling — they change around public holidays.*
