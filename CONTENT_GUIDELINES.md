...
- A new guide is linked both ways before it's finished: it links to the guides a reader would want next, and every related guide, hub and planning page links to it. Being linked from one page isn't enough. Check the site's own pages as well as articles: `src/pages` (/stay/, /guides/, /free/, the homepage) and the nav in `src/sites/london.ts`. A page that lists a family of guides should pick up a new one automatically; if it doesn't, make it. For a stay guide that means `best-areas-to-stay-in-london`, its area guide (both ways) and the neighbouring areas' guides; for a food guide, `eat-in-london-guide`; for a day trip, `day-trips-from-london`. `node scripts/audit-links.mjs` and `node scripts/check-hub-links.mjs` must pass; the build runs the second.
