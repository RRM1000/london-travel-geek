...
// data/consensus/italian.json. Anything that cannot be derived is an error.
//
// Counts are TOPIC-SCOPED: evidence.json's sourceCount spans all 48 corpora, and
// a single guide must not print that.
import fs from "node:fs";

const art = fs.readFileSync("src/content/articles/best-italian-restaurants-london.md", "utf8");
const ev = JSON.parse(fs.readFileSync("data/evidence.json", "utf8"));
const doc = JSON.parse(fs.readFileSync("data/consensus/italian.json", "utf8"));
...
// Italian publishes no ranked list either. Its only printable badge is the
// Michelin star, checked against the Michelin record's own quotes.
...
for (const line of lines) {
  ...
  const scoped = rec.byTopic?.italian?.sourceCount ?? rec.sourceCount;
  if (scoped !== claimed)
    errors.push(`${lastName}: article says ${claimed}, italian corpus says ${scoped}`);
  else checked.push(`${lastName}: ${claimed} OK`);

  // Michelin stars are the only badge. Check them against the Michelin record.
...
const italianVenues = Object.values(ev).filter((v) => v.topics.includes("italian"))
  .map((v) => ({ ...v, ...v.byTopic.italian }));
const actual = {
  sources: doc.sources.length,
  citations: doc.sources.reduce((n, s) => n + (s.names ?? []).length, 0),
  venues: italianVenues.length,
  twoPlus: italianVenues.filter((v) => v.sourceCount >= 2).length,
  awards: italianVenues.filter((v) => v.hasAward).length,
};
