// Checks every "Cited by N sources" and every printed rank in the dessert
// article against data/evidence.json and the sources' own quotes in
// data/consensus/dessert.json.
//
...
const TOPIC = "dessert";
const art = fs.readFileSync("src/content/articles/best-bakeries-london.md", "utf8");
...
const m = art.match(/\*\*(\d+) sources carrying (\d+) citations\*\* across \*\*(\d+) named bakeries\*\*/);
const m2 = art.match(/\*\*(\d+) bakeries are named by two or more independent sources/);
