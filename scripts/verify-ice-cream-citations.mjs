// Checks every "Cited by N sources" and every printed rank in the ice-cream
// article against data/evidence.json and the sources' own quotes in
// data/consensus/ice-cream.json.
//
...
const art = fs.readFileSync("src/content/articles/best-ice-cream-london.md", "utf8");
...
const m = art.match(/\*\*(\d+) sources carrying (\d+) citations\*\* across \*\*(\d+) named shops\*\*/);
const m2 = art.match(/\*\*(\d+) shops are named by two or more independent sources/);
