// Checks every "Cited by N sources" and every printed rank in the afternoon-tea
// article against data/evidence.json and the sources' own quotes in
// data/consensus/afternoon-tea.json.
//
// Counts are TOPIC-SCOPED: evidence.json's sourceCount spans every corpus, and
// a single guide must not print that.
import fs from "node:fs";

const TOPIC = "afternoon-tea";
const art = fs.readFileSync("src/content/articles/best-afternoon-tea-london.md", "utf8");
...
for (const line of art.split(/\r?\n/)) {
  ...
  // A "named by no source in this corpus" line is a claim too, and an easier one
  // to get wrong than a count - it goes stale the moment a source is added.
  if (/named by no source in this corpus/.test(line) && lastName) {
    const rec = evByNorm[resolve(lastName)];
    const n = rec?.byTopic?.[TOPIC]?.sourceCount ?? 0;
    if (n > 0) errors.push(`${lastName}: article says no source names it, corpus says ${n}`);
...
const m = art.match(/\*\*(\d+) sources carrying (\d+) citations\*\* across \*\*(\d+) named tea rooms\*\*/);
const m2 = art.match(/\*\*(\d+) tea rooms are named by two or more independent sources/);
