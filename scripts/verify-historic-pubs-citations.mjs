// Checks every "Cited by N sources" and every printed rank in the historic-pubs
// article against data/evidence.json and the sources' own quotes in
// data/consensus/historic-pubs.json.
//
// Counts are TOPIC-SCOPED: evidence.json's sourceCount spans every corpus, and
// a single guide must not print that.
import fs from "node:fs";

const TOPIC = "historic-pubs";
const art = fs.readFileSync("src/content/articles/historic-pubs-dining-rooms-london.md", "utf8");
...
for (const line of art.split(/\r?\n/)) {
  ...
  // A heading can be two things joined by "and" - try the full form first.
  if (h) {
    const head = h[1].replace(/\s+—.*$/, "").split(",")[0].trim();
    lastName = evByNorm[resolve(head)] ? head : head.split(/\s+and\s+/)[0].trim();
  }
  ...
  const claimed = line.match(/Cited by (\d+) sources?(?![A-Za-z])/)?.[1]
  ...
  // A CROSS-CORPUS citation: "Cited by 7 fish and chips sources". These pages
  // round up venues whose evidence lives in another topic, and the label names
  // which. Without this the line matches nothing and passes unchecked, which is
  // exactly the failure the checker exists to stop.
  const LABELS = { "fish and chips": "fish-and-chips", "Middle Eastern": "middle-eastern",
    "Sunday roast": "sunday-roast", "afternoon tea": "afternoon-tea", "dim sum": "dim-sum",
    "cheap eats": "cheap-eats", "historic pub": "historic-pubs", "ice cream": "ice-cream",
    breakfast: "breakfast-brunch" };
  const x = line.match(/Cited by (\d+) ([A-Za-z][A-Za-z- ]*?) sources?/);
  if (x && lastName) {
    const other = LABELS[x[2]] ?? x[2].toLowerCase().replace(/ /g, "-");
    const rec = evByNorm[resolve(lastName)];
    const real = rec?.byTopic?.[other]?.sourceCount;
    if (real == null) errors.push(`${lastName}: cited as "${x[2]}" but has no ${other} citations`);
    else if (real !== Number(x[1])) errors.push(`${lastName}: article says ${x[1]} ${other} sources, corpus says ${real}`);
    else checked.push(`${lastName}: ${x[1]} ${other} OK`);
  }
}
...
const m = art.match(/\*\*(\d+) sources carrying (\d+) citations\*\* across \*\*(\d+) named pubs and rooms\*\*/);
const m2 = art.match(/\*\*(\d+) pubs and rooms are named by two or more independent sources/);
