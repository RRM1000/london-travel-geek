// Checks every "Cited by N sources" and every printed rank in the breakfast-brunch
// article against data/evidence.json and the sources' own quotes in
// data/consensus/breakfast-brunch.json.
//
// Counts are TOPIC-SCOPED: evidence.json's sourceCount spans every corpus, and
// a single guide must not print that.
import fs from "node:fs";

const TOPIC = "breakfast-brunch";
const art = fs.readFileSync("src/content/articles/best-breakfast-brunch-london.md", "utf8");
...
const m = art.match(/\*\*(\d+) sources carrying (\d+) citations\*\* across \*\*(\d+) named rooms\*\*/);
const m2 = art.match(/\*\*(\d+) rooms are named by two or more independent sources/);
