// Checks every "Cited by N sources" in the Vietnamese article against
...
//
// TWO CLAIMS HERE ARE STRUCTURAL RATHER THAN DECORATIVE, and both rot silently:
//
//   1. Lai Rai's Bib Gourmand is the ONLY judged Vietnamese award in the UK and
//      Ireland. The opening rests on it and so does that entry. The corpus note
//      previously said no judged award existed at all, so this topic has
//      already been wrong about awards once.
//   2. The two most-cited Vietnamese restaurants in London are both on
//      Kingsland Road. The "start there, but do not finish there" section is
//      built on that being true, and a refresh that promotes an off-strip venue
//      would make the section argue against its own evidence.
import fs from "node:fs";

const TOPIC = "vietnamese";
const ARTICLE = "src/content/articles/best-vietnamese-restaurants-london.md";
...
// CLAIM 1 - the Bib Gourmand is the only judged award on this topic.
const awarded = venues.filter((v) => v.hasAward);
const claimsOnlyAward = /only Vietnamese restaurant in the UK or Ireland with a Michelin Bib Gourmand/.test(art);
if (claimsOnlyAward && awarded.length !== 1) {
  const names = awarded.map((v) => v.name).join(", ");
  errors.push(
    `the article calls Lai Rai the only Vietnamese Bib Gourmand, but ${awarded.length} venue(s) now carry an award (${names}). Rewrite the opening and that entry.`,
  );
}

// CLAIM 2 - the top two are both on Kingsland Road.
if (/the two most-cited Vietnamese restaurants in London are both on it/.test(art)) {
  const top2 = [...venues].sort((a, b) => b.sourceCount - a.sourceCount).slice(0, 2).map((v) => v.name);
  const onStrip = [/S(ô|o)ng Qu(ê|e)/i, /Vi(ệ|e)t Grill/i];
  if (!top2.every((n) => onStrip.some((re) => re.test(n)))) {
    errors.push(
      `the article says the two most-cited are both on Kingsland Road, but they are now ${top2.join(" and ")}. Rewrite the Kingsland Road section.`,
    );
  }
}
...
console.log(`award check: ${claimsOnlyAward ? "claim present and tested" : "claim absent"}`);
if (errors.length) {
