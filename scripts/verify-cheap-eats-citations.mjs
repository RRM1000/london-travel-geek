// Checks every "Cited by N sources" and every printed rank in the cheap-eats
// article against data/evidence.json and the sources' own quotes in
// data/consensus/cheap-eats.json.
//
// Counts are TOPIC-SCOPED: evidence.json's sourceCount spans every corpus, and
// a single guide must not print that.
import fs from "node:fs";

const TOPIC = "cheap-eats";
const art = fs.readFileSync("src/content/articles/cheap-eats-london.md", "utf8");
...
const m = art.match(/\*\*(\d+) sources carrying (\d+) citations\*\* across \*\*(\d+) named places\*\*/);
const m2 = art.match(/\*\*(\d+) places are named by two or more independent sources/);
